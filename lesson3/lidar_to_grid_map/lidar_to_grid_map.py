...
def init_flood_fill(center_point, obstacle_points, xy_points, min_coord,
    ...
    """
    center_x, center_y = center_point  # center point
    prev_ix, prev_iy = center_x - 1, center_y  # 中心左侧一个栅格
    ox, oy = obstacle_points  # 障碍物点
    xw, yw = xy_points  # 栅格地图尺寸
    min_x, min_y = min_coord  # 地图左下角坐标，用来偏移
    occupancy_map = (np.ones((xw, yw))) * 0.5  # 初始置为 未知=非0/1
    for (x, y) in zip(ox, oy):
        # x coordinate of the the occupied area
        ix = int(round((x - min_x) / xy_resolution))
        # y coordinate of the the occupied area
        iy = int(round((y - min_y) / xy_resolution))
        free_area = bresenham((prev_ix, prev_iy), (ix, iy))  # 射线区域，未占用
        for fa in free_area:
...
def main():
    ...
    #b1 = time.time()
    #for i in range(100):
    occupancy_map_b, min_x, max_x, min_y, max_y, xy_resolution = \
    ...
    #print("breshenham time: ", (b2 - b1) / 100)
    #print("flood fill time: ", (b3 - b2) / 100)

    xy_res = np.array(occupancy_map).shape
    plt.figure(1, figsize=(12, 4))
    plt.subplot(133)  # flood fill
    ...
    plt.subplot(132)  # breshenham
    plt.title("breshenham")
    plt.imshow(occupancy_map_b, cmap="PiYG_r")
    # cmap = "binary" "PiYG_r" "PiYG_r" "bone" "bone_r" "RdYlGn_r"
    plt.clim(-0.4, 1.1)
    plt.gca().set_xticks(np.arange(-.5, xy_res[1], 1), minor=True)
    plt.gca().set_yticks(np.arange(-.5, xy_res[0], 1), minor=True)
