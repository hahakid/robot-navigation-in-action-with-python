...
# Parameters
k = 0.1  # look forward gain  # 预瞄增益
Lfc = 2.0 # 2 # [m] look-ahead distance   # 预瞄点距离
Kp = 1.0  # speed proportional gain， 速度比例增益
dt = 0.1  # [s] time tick, 时间步长
WB = 2.9  # [m] wheel base of vehicle, 车轴距离


# Vehicle parameters
LENGTH = WB + 1.0  # Vehicle length， 车辆模型，车长
WIDTH = 2.0  # Vehicle width， 车辆模型，车宽
WHEEL_LEN = 0.6  # Wheel length， 车轮模型，车轴到车轮的距离
WHEEL_WIDTH = 0.2  # Wheel width， 车轮模型，车轮宽度
MAX_STEER = math.pi / 4  # Maximum steering angle [rad]， 最大转向角， 45度
...
class State:
    ...
    def calc_distance(self, point_x, point_y):
        ...
        # 计算两点之间的直线距离
        dx = self.rear_x - point_x  # 计算x轴上的距离
        dy = self.rear_y - point_y  # 计算y轴上的距离
        return math.hypot(dx, dy)  # 直线距离
    ...
    def search_target_index(self, state):
        ...
        if self.old_nearest_point_index is None:
            ...
            distance_this_index = state.calc_distance(self.cx[ind],
                                                      self.cy[ind])
            while True:
                if ind + 1 >= len(self.cx):  # 最后一帧，防止越界
                    break
...
def main():
    ...
    T = 100.0  # max simulation time  仿真时间

    # initial state
    state = State(x=-0.0, y=0.0, yaw=math.pi if is_reverse_mode else 0.0, v=0.0, is_reverse=is_reverse_mode)
