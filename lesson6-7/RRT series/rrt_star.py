...
class RRTStar(RRT):
    ...
    def __init__(self,
                 ...
                 rand_area,
                 expand_dis=3,  # 30.0,
                 path_resolution=0.5, #1.0,
                 goal_sample_rate=5,  #  20,
                 max_iter=500,
                 connect_circle_dist=50.0,  # added for rrt*
                 search_until_max_iter=False,  # added for rrt*
                 robot_radius=0.0):
        """
        Setting Parameter
    ...
    def planning(self, animation=True):
        ...
        for i in range(self.max_iter):
            ...
            nearest_ind = self.get_nearest_node_index(self.node_list, rnd_node)  # 索引
            near_node = self.node_list[nearest_ind]  # 对应节点
            new_node = self.steer(near_node, rnd_node, self.expand_dis)  # 扩展

            new_node.cost = near_node.cost + \
                math.hypot(new_node.x-near_node.x, new_node.y-near_node.y)  # 代价
