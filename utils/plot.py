...
def get_frame_as_array(fig):
    """将Matplotlib图形转换为RGB格式的NumPy数组"""
    buf = BytesIO()
    fig.savefig(buf, format='png')  # 改为RGB格式
    buf.seek(0)
    img = Image.open(buf)
    img_arr = np.array(img)  # np.frombuffer(buf.getvalue(), dtype=np.uint8)
    buf.close()

    #h, w = fig.canvas.get_width_height()
    #img = img_arr.reshape(h, w, -1)  # RGBA格式（4通道）
