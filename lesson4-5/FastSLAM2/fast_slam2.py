...
start_waiting = 3.0
DT = 0.5  # time tick [s]
SIM_TIME = 63.0 + start_waiting # simulation time [s]
...
def proposal_sampling(particle, z, Q_cov):
    ...
    dz = z[0:2].reshape(2, 1) - zp  # 残差
    dz[1] = pi_2_pi(dz[1])  # 归一化
    ...
    for iz in range(len(z[0, :])):
        ...
        for ip in range(N_PARTICLE):
            ...
            # known landmark
                ...
                particles[ip] = proposal_sampling(particles[ip], z[:, iz], Q)  # 又增加了一个新的计算
...
def main():
    ...
    while SIM_TIME >= time:
        ...
        if show_animation:  # pragma: no cover
            ...
            for iz in range(len(z[:, 0])):
                landmark_id = int(z[2, iz])
                #plt.plot([x_est[0][0], rfid[landmark_id, 0]], [x_est[1][0], rfid[landmark_id, 1]], "-k")
                best_particle = max(particles, key=lambda p: p.w)  # 基于最优粒子估计路标绘制观测连线
                lx, ly = best_particle.lm[landmark_id, 0], best_particle.lm[landmark_id, 1]
                plt.plot([x_est[0][0], lx], [x_est[1][0], ly], "-k")
